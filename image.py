...
class Image(object):
    ...
    def __init__(self,image_size=None, arcsec_pix=None, rmax_arcsec=None,
        ...
        self.rmax = int(rmax_arcsec / arcsec_pix)
        self.model = model
    ...
    def los_image_full(self,p):
        ...
        # z and y arrays
        a = np.arange(crop_size) - (crop_size-1)/2.
        zarray, yarray = np.meshgrid(a, a)
        yarray -= y0

        # x-independent parts of the coordinate transformation
        trans1 = -(s0*c1*c2 + c0*s2)*yarray + s1*c2*zarray
        trans2 = -(s0*c1*s2 - c0*c2)*yarray + s1*s2*zarray
        ...
        for i in np.arange(crop_size):
            ...
            x3 = (c0c1c2 - s0s2)*x + trans1
            y3 = (c0c1s2 + s0c2)*x + trans2
            z3 = -c0s1*x + trans3

            # get the spherical polars
            rxy2 = x3*x3 + y3*y3
            rxy = np.sqrt(rxy2)
            r = np.sqrt(rxy2 + z3*z3) * self.arcsec_pix
            az = np.arctan2(y3,x3)
            el = np.arctan2(z3,rxy)

            # the density in this y,z later
            layer = self.emit(r) * self.dens(r,az,el,p[6:])

            # put this in the image
            image[ny2-self.rmax:ny2+self.rmax, nx2-self.rmax+i] = \
                                np.sum(layer,axis=1)
